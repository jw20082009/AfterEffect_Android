 ...
 * 视频帧裁剪页面
 */
public abstract class VideoCutViewBar extends View {
    ...
    // 帧提取器
    protected IMetadataRetriever mMetadataRetriever;
    ...
    public VideoCutViewBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMetadataRetriever = getMetadataRetriever();
        mLoadingFrames = new ArrayList<>();
        new LoadThread().start();
    }

    public abstract IMetadataRetriever getMetadataRetriever();
    ...
    public void setVideoPath(String videoPath) {
        ...
        try {
            ...
            mVideoWidth = mMetadataRetriever.getVideoWidth();
            mVideoHeight = mMetadataRetriever.getVideoHeight();
            mVideoRotation = mMetadataRetriever.getVideoRotation();
            mVideoDuration = mMetadataRetriever.getVideoDuration();
            ...
            loadFail = true;
            if (mOnOnVideoCropViewBarListener != null) {
                mOnOnVideoCropViewBarListener
                        .onError(e.getMessage() == null ? "not error message" : e.getMessage());
            }
        }
    }
    ...
    private void processEventMove() {
        ...
        if (mCurrentNode != null && mCurrentNode.getFrameTime() == 0 && mExcursionX < 0) {
            ...
            if (mOnOnVideoCropViewBarListener != null) {
                mOnOnVideoCropViewBarListener.onTouchChange((long) ((mCurrentNode == null ? 0
                        : mCurrentNode.getFrameTime() + (mExcursionX / mViewFrameWidth
                                * (mFrameTime * mCurrentSpeed.getSpeed())))
        ...
        if (mVideoDuration != 0
                && ((mLastNode != null
                        && mLastNode.getFrameTime()
                                + (mFrameTime * mCurrentSpeed.getSpeed()) > mVideoDuration
                        && mExcursionX > 0)
                        || (mCurrentNode != null
                ...
                || (mCurrentNode != null
                        && mCurrentNode.getFrameTime()
                                + (mCropEnd - mCropStart) * mCurrentSpeed.getSpeed()
                                + (mFrameTime * mCurrentSpeed.getSpeed()) > mVideoDuration
                        && mExcursionX > 0)) {
            mExcursionX = 0;
            if (mOnOnVideoCropViewBarListener != null) {
            ...
            if (mExcursionX > mViewFrameWidth) {
                synchronized (mCurrentNode) {
                    if (mCurrentNode.next() == null) {
                        ...
                        node.setFrameTime((long) (mCurrentNode.getFrameTime()
                                + (mFrameTime * mCurrentSpeed.getSpeed())));
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (touchEdge) {
                    if (touchRight) {
                        mCropEnd -= (mMoveX - event.getX())
                                / (getMeasuredWidth() / (float) mMaxTime);
                    ...
                    if (mOnOnVideoCropViewBarListener != null) {
                        mOnOnVideoCropViewBarListener
                                .onRangeChange(
                                        (long) ((mCurrentNode == null ? 0
                                                : mCurrentNode.getFrameTime() + (mExcursionX
                                                        / mViewFrameWidth
                                                        * (mFrameTime * mCurrentSpeed.getSpeed())))
                                                + mCropStart * mCurrentSpeed.getSpeed()),
        ...
        for (int i = -1; i < frameSize; i++) {
            ...
            if (node.getBitmap() != null && !node.getBitmap().isRecycled()) {
                ...
                if (node.getFrameTime() + mFrameTime * mCurrentSpeed.getSpeed() > mVideoDuration) {
                    right = (int) (left + (mViewFrameWidth * (mVideoDuration - node.getFrameTime())
                            / (mFrameTime * mCurrentSpeed.getSpeed())));
                    srcRect = new Rect(0, 0,
                            (int) (mViewFrameWidth * (mVideoDuration - node.getFrameTime())
                                    / (mFrameTime * mCurrentSpeed.getSpeed())),
                            getMeasuredHeight());
                }
            ...
            if (node.next() == null) {
                ...
                frameNode.setFrameTime(
                        (long) (node.getFrameTime() + (mFrameTime * mCurrentSpeed.getSpeed())));
    ...
    public Bitmap getVideoFrame(long frameNumber) {
        ...
        return mMetadataRetriever.getScaledFrameAtTime(frameNumber * 1000L, (int) mViewFrameWidth,
                (int) (mViewFrameWidth / mVideoSar));
    }
